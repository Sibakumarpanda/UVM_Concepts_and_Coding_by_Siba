...
-start method definition:            virtual task start ( uvm_sequencer_base sequencer,
...
- Where , sequencer = While starting a sequence, on which sequencer it has to be started, need to be specified whereas other arguments are optional.
          parent_sequence = parent_sequence is a sequence that calls the current sequence. 
                            If parent_sequence is null, then this sequence is a root parent otherwise, it is a child of parent_sequence. 
                            The parent_sequence’s pre_do, mid_do, and post_do methods will be called during the execution of the current sequence.
          this_priority  =  this_priority is the priority of the sequence (by default it considers the priority of the parent sequence). 
                            Higher priority is indicated by higher value.
          call_pre_post =   By default, call_pre_post = 1 is set which means pre_body and post_body methods will be called. 
                            To disable calling these methods, call_pre_post can be set to 0.
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------
task                                         pre_start                                        It is a user-definable callback that is called before the 
                                                                                              optional execution of the pre_body task.  

task                                         pre_body                                         It is a user-definable callback that is called before the 
                                                                                              execution of body only when the sequence is started with start.

task                                         pre_do                                           It is a user-definable callback task that is called on parent sequence (if any)
                                                                                              before the item is randomized and after sequence has issued
                                                                                              wait_for_grant() call.

function                                     mid_do                                           It is a user-definable callback function that is called after the sequence item 
                                                                                              is randomized, and just before the item is sent to the driver.
  
task                                         body                                             It is a user-defined task to write main sequence code.

function                                     post_do                                          It is a user-definable callback function that is called after completing 
                                                                                              the item using either put or item_done methods.

task                                         post_body                                        It is a user-definable callback task that is called after the execution 
                                                                                              of the body only when the sequence is started with the start method.
  
task                                         post_start                                       It is a user-definable callback that is called after the optional 
                                                                                              execution of the post_body task.
