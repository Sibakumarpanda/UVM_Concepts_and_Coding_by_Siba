...
-Note that already we are aware that , active agent contais the three component as sequencer , driver , monitor .
 And Passive component contains only Monitor .
-These sequence items or transactions are broadcasted to other components like the UVM scoreboard, coverage collector etc.
-Monitor uses a TLM analysis port to broadcast transactions.
...
////////////////////////////
   Purpose of Monitor
/////////////////////////////  
-Monitor Capture dut signal level information and translate/convert it into the transaction.
-Broadcast transactions to other components using TLM analysis port for coverage collection and checking purposes (scoreboard).
...
-Since monitor is a uvm_component ,the new() function has two arguments as string name and uvm_component parent.
