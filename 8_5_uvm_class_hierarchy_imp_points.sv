...
-It is an abstract class that does not have any data members or methods implemented. 
-It is a generic container for objects to be created similar to void pointer in C language.
-Syntax: virtual class uvm_void
  
/////////////////
  uvm_object 
////////////////  
-The UVM object is a data structure used for testbench configuration
-It is the base class available for component and sequence branch.
-The uvm_object provides methods like create, print, copy, clone, compare, record etc.
  
///////////////////////////
  uvm_report_object
///////////////////////////  
-The uvm_report_object provides reporting functionality for UVM. 
-The message, error ,warning prints are very important for debugging purposes that are being facilitated by the uvm_report_object class.
...
uvm_verbosity:
-UVM defines standard verbosity for report displays. 
-Note : In below , priority order will be from Highest (UVM_NONE) to Lowest (UVM_DEBUG)
...
-Since uvm_top is globally accessible in uvm_pkg scope.
-UVM’s reporting mechanism is also accessible from anywhere (even outside of uvm_component) like sequences or objects.
