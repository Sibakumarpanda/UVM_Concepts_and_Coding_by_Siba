//////////////////////////////////////////////////////////////
   Concept of m_sequencer and p_sequencer
...
- Example: base_seq.start(env_o.seqr);  // Remember ? we will write this in test class run phase:  seq.start(env.agnt.seqr);
...
`define uvm_declare_p_sequencer(SEQUENCER) \
  ...
  virtual function void m_set_p_sequencer();\
    ...
    if( !$cast(p_sequencer, m_sequencer)) \
      `uvm_fatal("DCLPSQ", $sformatf("%m %s Error casting p_sequencer, please verify that this sequence/sequence item is intended to 
                                     execute on this type of sequencer",get_full_name())) \
  endfunction 
...
-This is used for user convenience. 
-It is not a mandatory requirement. You can access virtual_sequencer with the env hierarchy path.
-It sounds confusing ,but to understand the usage of p_sequencer and what exactly it is.We need to undertand virtual sequence and virtual sequencer concept. 
-Let’s understand what is a virtual sequence and virtual sequencer concept in the next section.     
